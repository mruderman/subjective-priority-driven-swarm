...
def list_existing_agents(client):
    ...
    try:
        ...
        for agent in agents:
            ...
            print(
                f"    Created: {str(agent.created_at)[:10] if hasattr(agent, 'created_at') else 'Unknown'}"
            )
        return agents
    except Exception as e:
...
def test_secretary_creation(client):
    ...
    try:
        ...
        print(
            f"✅ Secretary created: {secretary.agent.name if secretary.agent else 'Unknown'}"
        )
        ...
        secretary.observe_message(
            "Test User", "This is a test message for the secretary."
        )
...
def test_swarm_manager(client):
    ...
    try:
        ...
        manager._update_agent_memories(
            "This is a test message about debugging", "Test User"
        )
