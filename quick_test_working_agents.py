...
def test_working_agents():
    ...
    working_agent_names = ["Alex - Project Manager", "companion-agent-1753201615269-sleeptime"]
    ...
    for agent in working_agents:
        ...
        try:
            ...
            print(f"  📊 Motivation: {spds_agent.motivation_score}, Priority: {spds_agent.priority_score:.2f}")
        except Exception as e:
        ...
        if spds_agent.priority_score > 0:
            try:
                response = spds_agent.speak(mode="initial", topic="AI collaboration in teams")
                ...
                if not message_text:
                    message_text = "I have some thoughts but I'm having trouble expressing them."
                
                print(f"  💬 Response: {message_text[:200]}{'...' if len(message_text) > 200 else ''}")
...
def test_with_swarm_manager():
    ...
    for agent in agents:
        if agent.name in ["Alex - Project Manager", "companion-agent-1753201615269-sleeptime"]:
            try:
                # Quick health check
                client.agents.messages.create(
                    agent_id=agent.id,
    ...
    try:
        ...
        if motivated_agent:
            print(f"✅ {motivated_agent.name} is motivated (priority: {motivated_agent.priority_score:.2f})")
