...
def test_agent_responses():
    ...
    usable_agents = [
        a for a in agents if any(name in a.name for name in ["Alex", "Jordan", "Casey"])
    ]
    ...
    print("\n=== Test 1: Simple Question ===")
    test_question = (
        "What do you think is the most important skill for effective teamwork?"
    )
    ...
    for i, agent in enumerate(manager.agents):
        ...
        print(
            f"  Motivation: {agent.motivation_score}, Priority: {agent.priority_score:.2f}"
        )
        ...
        if agent.priority_score > 0:
            try:
                ...
                print(
                    f"  Response: {message_text[:150]}{'...' if len(message_text) > 150 else ''}"
                )
    ...
    print(
        f"Initial responses: {success_count}/{total_agents} agents gave good responses"
    )
    print(
        f"Follow-up responses: {followup_success}/{total_agents} agents gave good follow-ups"
    )
