...
class ResponseValidator:
    ...
    def test_initial_response_failure(self):
        ...
        for i, agent in enumerate(motivated_agents):
            ...
            try:
                ...
                response_analysis = self._analyze_response(agent.name, message_text, response)
        ...
        print(f"  Initial responses: {len(initial_responses) - failure_count}/{len(motivated_agents)} successful")
        print(f"  Subsequent responses: {subsequent_success}/{len(motivated_agents)} successful")
        ...
        return len(self.failure_patterns) > 0  # Return True if we found failure patterns
    ...
    def test_message_format_validation(self):
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                print(f"    Status: {'✅' if analysis['valid'] else '❌'} {analysis['summary']}")
    ...
    def test_state_persistence(self):
        ...
        self.client.agents.messages.create(
            agent_id=agent.agent.id,
            messages=[{"role": "user", "content": "User: Let's discuss innovation strategies in tech teams."}]
        )
        ...
        self.client.agents.messages.create(
            agent_id=agent.agent.id,
            messages=[{"role": "user", "content": "User: What specific innovation practices work best?"}]
        )
        ...
        # Check persistence
        state_changed = (
            first_assessment['motivation'] != second_assessment['motivation'] or
            first_assessment['priority'] != second_assessment['priority']
        )
    ...
    def _analyze_response(self, agent_name: str, message_text: str, response: LettaResponse) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'response_raw': str(response)[:200] + '...' if len(str(response)) > 200 else str(response)
        }
        ...
        if hasattr(response, 'messages') and response.messages:
            for msg in response.messages:
                analysis['message_types'].append(msg.role if hasattr(msg, 'role') else 'unknown')
    ...
    def _analyze_response_format(self, response: LettaResponse) -> Dict[str, Any]:
        ...
        try:
            if hasattr(response, 'messages') and response.messages:
                ...
                analysis['summary'] = f"{len(response.messages)} messages, {len(set(analysis['message_types']))} types"
            else:
    ...
    def _generate_validation_report(self):
        ...
        if 'initial_response_test' in self.test_results:
            ...
            print(f"  Failure rate: {(test1['initial_failures']/test1['total_agents']*100):.1f}%")
            ...
            if test1['failure_patterns']:
                ...
                for i, pattern in enumerate(test1['failure_patterns'][:3], 1):
                    ...
                    if 'analysis' in pattern:
                        ...
                        print(f"     Content length: {pattern['analysis']['content_length']}")
        ...
        if 'state_persistence' in self.test_results:
            ...
            print(f"  Assessment object valid: {'✅' if test3['assessment_valid'] else '❌'}")
        ...
        if 'format_validation' in self.test_results:
            failed_formats = [r for r in self.test_results['format_validation'] if not r.get('success', True)]
            if failed_formats:
