...
class ExportManager:
    ...
    def __init__(self, export_directory: str = None):
        self.export_directory = Path(export_directory or config.DEFAULT_EXPORT_DIRECTORY)
    ...
    def export_action_items(
        ...
        if not action_items:
            ...
            for i, item in enumerate(action_items, 1):
                description = item.get('description', item.get('content', f'Action item {i}'))
    ...
    def export_executive_summary(
        ...
        content += f"- **Engagement**: {stats.get('total_messages', 0)} total contributions\n"
        ...
        effectiveness = "High" if len(decisions) > 1 or len(action_items) > 2 else "Moderate"
    ...
    def export_complete_package(
        ...
        # Structured data
        data_file = self.export_structured_data(
            meeting_data, f"{base_filename}_data"
        )
