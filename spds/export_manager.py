...
class ExportManager:
    ...
    def export_meeting_minutes(
        ...
        """Export meeting minutes. Content is generated by the secretary agent."""
        ...
        # Secretary agent generates the actual minutes content via AI.
        # meeting_data may contain pre-formatted content or structured data.
        content = meeting_data.get("content", "")
        if not content:
            content = meeting_data.get("minutes", "")
        if not content:
            # Fallback: dump the meeting data as formatted text
            topic = meeting_data.get("metadata", {}).get("topic", "Meeting")
            content = f"# {format_type.title()} Minutes: {topic}\n\n"
