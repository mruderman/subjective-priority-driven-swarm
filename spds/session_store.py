    ...
    def save_event(self, event: SessionEvent) -> None:
        ...
        with lock:
            ...
            try:
                ...
                if session_file.exists():
                    try:
                        with session_file.open("r") as f:
                            content = f.read()
                            if content.strip():
                                session_state = SessionState.model_validate_json(
                                    content
                                )
                        ...
                        logger.debug(
                            f"Cannot load session.json directly: {e}, skipping update"
                        )
                else:
                    logger.debug(
                        f"Session file {session_file} does not exist, creating minimal session.json"
                    )
                ...
                logger.warning(
                    f"Failed to update session.json for {session_id}, but event saved to events.jsonl: {e}"
                )
                ...
                try:
                    with session_file.open("r") as f:
                        ...
                        if content.strip():
                            ...
                            logger.debug(
                                f"Loaded session {session_id} from session.json"
                            )
                            return session_state
                except Exception as e:
                ...
                try:
                        ...
                        for line in f:
                            ...
                            if line:
                                try:
                                    ...
                                    logger.warning(
                                        f"Skipping malformed event line: {e}"
                                    )
                    ...
                    if events:
                        ...
                        for event in events:
                            if (
                                event.type == "system"
                                and event.payload.get("event_type") == "session_created"
                            ):
                    ...
                    logger.error(
                        f"Failed to rebuild session from events.jsonl for {session_id}: {e}"
                    )
