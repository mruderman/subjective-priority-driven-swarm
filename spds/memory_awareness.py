...
class MemoryAwarenessReporter:
            ...
            if hasattr(core_memory, 'memory'):
                for block_label, block_content in core_memory.memory.items():
                    ...
                    memory_blocks[block_label] = {
                        ...
                        'content_preview': str(block_content)[:100] + '...' if block_size > 100 else str(block_content)
                    }
    ...
    def _generate_objective_observations(self, recall_count: int, archival_count: int, 
                                       core_memory_size: int) -> List[Dict[str, Any]]:
        """
    ...
    def should_provide_memory_awareness(self, agent: AgentState, last_check: Optional[datetime] = None) -> bool:
        """
    ...
    def format_neutral_awareness_message(self, metrics: Dict[str, Any]) -> str:
        ...
        message += f"- Recall Memory: {memory_metrics['recall_memory_count']} messages\n"
        message += f"- Archival Memory: {memory_metrics['archival_memory_count']} items\n"
        message += f"- Core Memory: {memory_metrics['core_memory_total_chars']} characters\n\n"
...
def create_memory_awareness_for_agent(client: Letta, agent: AgentState) -> Optional[str]:
    """
