...
"""
        ...
        # Call to order
        minutes += "\n### CALL TO ORDER\n"
        minutes += f"The meeting was called to order at {start_time.strftime('%I:%M %p')}.\n"
        ...
        if conversation_log:
            ...
            minutes += self._generate_formal_discussion_summary(conversation_log, participants)
            minutes += "\n"
        ...
        if stats:
            ...
            minutes += f"- **Active Participants**: {len(stats.get('participants', {}))}\n"
            minutes += f"- **Decisions Made**: {stats.get('decisions', 0)}\n"
            minutes += f"- **Action Items Created**: {stats.get('action_items', 0)}\n"
        
        return minutes
    
    def _generate_formal_discussion_summary(self, conversation_log: List[Dict], participants: List) -> str:
        ...
        substantive_messages = [entry for entry in conversation_log 
                              if len(entry.get("message", "")) > 20]
        ...
        if "strategy" in all_text.lower():
            summary += "Strategic considerations were emphasized throughout the discussion. "
...
"""
        ...
        if conversation_log:
            minutes += self._generate_casual_discussion_summary(conversation_log, metadata.get('participants', []))
        else:
        ...
        if stats and stats.get('total_messages', 0) > 0:
            minutes += f"\n## 📊 Quick Stats\n"
            minutes += f"- **Total messages**: {stats.get('total_messages', 0)}\n"
            minutes += f"- **Most chatty**: {self._get_most_active_participant(stats)}\n"
    ...
    def _determine_conversation_vibe(self, conversation_log: List[Dict], decisions: List, action_items: List) -> str:
    ...
    def _generate_casual_discussion_summary(self, conversation_log: List[Dict], participants: List) -> str:
    ...
    def _extract_casual_insights(self, conversation_log: List[Dict]) -> List[str]:
        ...
        insight_indicators = ["I think", "what if", "maybe we should", "the key is", "important point"]
    ...
    def _extract_good_ideas(self, conversation_log: List[Dict]) -> List[str]:
        ...
        idea_indicators = ["idea", "solution", "approach", "strategy", "could", "might", "what about"]
