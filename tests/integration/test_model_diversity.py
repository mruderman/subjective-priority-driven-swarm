...
class TestModelDiversity:
    ...
    def test_swarm_creation_with_diverse_models(
        self, mock_create_new, mock_letta_client, diverse_agent_profiles
    ):
        ...
        manager = SwarmManager(
            client=mock_letta_client, agent_profiles=diverse_agent_profiles
        )
        ...
        for i, (call_args, profile) in enumerate(
            zip(mock_create_new.call_args_list, diverse_agent_profiles)
        ):
            kwargs = call_args[1]
    ...
    def test_creative_swarm_json_loading(self, mock_letta_client):
        ...
        assert (
            len(model_providers) >= 3
        ), f"Should have diverse providers, got: {model_providers}"
    ...
    @patch("spds.swarm_manager.SPDSAgent.create_new")
    def test_model_specific_conversation_flow(
        self, mock_create_new, mock_letta_client, diverse_agent_profiles
    ):
                ...
                messages=[
                    SimpleNamespace(
                        ...
                        content=[
                            {
                                "type": "text",
                                "text": f"Response from {profile['model']} agent",
                            }
                        ],
                    )
                ]
        ...
        assert (
            "Response from google/gemini-pro-1.5 agent" in manager.conversation_history
        )
    ...
    def test_full_diverse_swarm_workflow(self, mock_create_new, mock_letta_client):
                ...
                messages=[
                    SimpleNamespace(
                        ...
                        content=[
                            {
                                "type": "text",
                                "text": f"Creative response from {profile['name']}",
                            }
                        ],
                    )
                ]
        ...
        manager = SwarmManager(
            client=mock_letta_client, agent_profiles=creative_profiles
        )
