...
def _mgr_with_dummy_agents(n=2):
    ...
    with patch("spds.swarm_manager.SPDSAgent.create_new") as create_new:
        ...
        for i in range(n):
            ...
            da.speak = lambda conversation_history="": SimpleNamespace(messages=[SimpleNamespace(role="assistant", content=[{"type": "text", "text": "ok"}])])
            agents.append(da)
        create_new.side_effect = agents
        profiles = [{"name": a.name, "persona": "p", "expertise": ["x"], "model": "openai/gpt-4", "embedding": "openai/text-embedding-ada-002"} for a in agents]
        mgr = SwarmManager(client=client, agent_profiles=profiles, conversation_mode="sequential")
        return mgr, client
...
def test_memory_awareness_command_prints_when_available():
    ...
    with patch("spds.swarm_manager.create_memory_awareness_for_agent", return_value="info"):
        captured = StringIO()
        sys.stdout = captured
