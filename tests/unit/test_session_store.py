...
from spds.session_store import JsonSessionStore, SessionEvent, SessionMeta, SessionState
...
class TestJsonSessionStore:
    ...
    def test_create_session(self, session_store):
        ...
        session_state = session_store.create(
            title="Test Session", tags=["test", "demo"]
        )
    ...
    def test_create_session_with_custom_id(self, session_store):
        ...
        session_state = session_store.create(
            session_id=custom_id, title="Custom Session"
        )
    ...
    def test_concurrent_access(self, session_store):
        ...
        assert len(loaded_state.events) == 1 + (
            3 * event_count
        )  # 1 session_created + 30 manual events
...
class TestSessionModels:
    ...
    def test_session_state_creation(self):
        ...
        state = SessionState(meta=meta, events=events, extras={"test": "extra data"})
