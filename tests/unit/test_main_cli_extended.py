...
def test_main_swarm_config_happy():
    profiles = [
        {"name": "A", "persona": "p", "expertise": ["x"], "model": "openai/gpt-4", "embedding": "openai/text-embedding-ada-002"},
        {"name": "B", "persona": "p", "expertise": ["y"], "model": "openai/gpt-4", "embedding": "openai/text-embedding-ada-002"},
    ]
    class DummySwarm:
        called = {}
        def __init__(self, **kwargs):
    ...
    with patch("spds.main.load_swarm_from_file", return_value=profiles), \
...
def test_main_interactive_cancel():
    with patch("spds.main.interactive_agent_selection", return_value=None), \
         patch("spds.main.Letta"):
        from spds.main import main
...
def test_main_cloud_api_key():
    with patch("spds.main.config.LETTA_ENVIRONMENT", "LETTA_CLOUD"), \
         patch("spds.main.config.LETTA_SERVER_PASSWORD", ""), \
         patch("spds.main.config.LETTA_API_KEY", "api-key"), \
