...
def test_speak_has_tools_initial_and_response():
    ...
    state = mk_agent_state(id="ag", name="N", system="S", model="openai/gpt-4", tools=[tool])
    agent = SPDSAgent(state, client)
    # First call returns tool-call flow; second is arbitrary
    client.agents.messages.create.return_value = SimpleNamespace(messages=[SimpleNamespace(role="assistant", content=[{"type": "text", "text": "ok"}])])
...
def test_speak_fallback_direct_instruction():
    ...
    state = mk_agent_state(id="ag", name="N", system="S", model="openai/gpt-4", tools=[tool])
    agent = SPDSAgent(state, client)
    client.agents.messages.create.side_effect = [Exception("No tool calls found"), SimpleNamespace(messages=[SimpleNamespace(role="assistant", content=[{"type": "text", "text": "ok"}])])]
    agent.speak("", mode="initial", topic="T")
