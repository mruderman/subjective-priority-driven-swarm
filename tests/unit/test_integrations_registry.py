...
class TestMCPProvider:
    ...
    def test_mcp_provider_discover_without_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=False):
            provider = MCPProvider()
    ...
    def test_mcp_provider_discover_with_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=True):
            provider = MCPProvider()
            tools = provider.discover()
            assert len(tools) == 2
    ...
    def test_mcp_provider_run_with_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=True):
            ...
            # Test translate tool
            result = provider.run("translate", {"text": "hello", "target_language": "es"})
...
class TestComposioProvider:
    ...
    def test_composio_provider_run_without_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=False):
            ...
            with pytest.raises(RuntimeError, match="Composio SDK not available"):
                provider.run("github_create_issue", {"repo": "test/repo", "title": "Test"})
    ...
    def test_composio_provider_run_with_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=True):
            ...
            result = provider.run("github_create_issue", {
                "repo": "test/repo", 
                "title": "Test Issue",
                "body": "Test body"
            })
            ...
            result = provider.run("slack_send_message", {
            ...
            result = provider.run("gmail_send_email", {
                "to": "test@example.com",
                "subject": "Test",
                "body": "Hello"
            })
    ...
    def test_composio_provider_run_nonexistent_tool(self):
        ...
        with patch('spds.config.get_integrations_allow_fake_providers', return_value=True):
            provider = ComposioProvider()
            with pytest.raises(KeyError, match="Fake Composio tool 'nonexistent' not found"):
                provider.run("nonexistent", {})
    ...
    def test_full_integration_flow_with_fake_providers(self):
        ...
        with patch('spds.config.get_integrations_enabled', return_value=True):
            with patch('spds.config.get_integrations_allow_fake_providers', return_value=True):
                ...
                translate_result = json.loads(tools["mcp.translate"]('{"text": "hello"}'))
                assert "translated" in translate_result["translated_text"]
                
                github_result = json.loads(tools["composio.github_create_issue"]('{"repo": "test/repo"}'))
